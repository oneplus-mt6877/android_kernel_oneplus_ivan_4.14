...
static int oplus_get_ntc_temp(struct iio_channel *ntc_temp_chan, int *ntc_temp)
{
	...
	*ntc_temp = con_temp_20131[i];
	...
	//chg_err("ntc_temp_volt:%d,temp:%d\n", ntc_temp_volt, *ntc_temp);
	return 0;
}
...
static int oplus_get_temp_volt(struct ntc_temp *ntc_param)
{
	...
	sub_temp_volt = sub_temp_volt * 1500 / 4096;
...
static ssize_t show_typec_sbu_voltage(struct device *dev, struct device_attribute *attr, char *buf)
{
	chg_err("get oplus_get_typec_sbu_voltage = [%d].\n",  oplus_get_typec_sbu_voltage());

	return sprintf(buf, "%d\n",  oplus_get_typec_sbu_voltage());
}
static ssize_t store_typec_sbu_voltage(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	return -1;
}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(oplus_chip->usb_psy) == 0) {
		ret_device_file = device_create_file(&oplus_chip->usb_psy->dev, &dev_attr_typec_sbu_voltage);
	}
	info->chg1_consumer =
...
#ifdef OPLUS_FEATURE_CHG_BASIC
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 0))
	.resume		= mtk_charger_resume,
	.suspend	= mtk_charger_suspend,
#endif
#endif
};
